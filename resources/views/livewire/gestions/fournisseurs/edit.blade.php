...
new class extends Component {
    ...
    {
        $endpoint = config('services.jwt.profile_endpoint') . '/fournisseur/fournisseur/' . $this->fournisseurId;

        $response = Http::withToken($this->token)->get($endpoint);

        if ($response->ok() && (!isset($response['error']) || !$response['error'])) {
            $data = $response['data'] ?? [];

            $this->code = $data['code'] ?? '';
            $this->name = $data['name'] ?? '';
            $this->raison_social = $data['raison_social'] ?? '';
            $this->adresse_siege = $data['adresse_siege'] ?? '';
            $this->code_postal = $data['code_postal'] ?? '';
            $this->ville = $data['ville'] ?? '';
            $this->telephone = $data['telephone'] ?? '';
            $this->fax = $data['fax'] ?? '';
            $this->mail = $data['mail'] ?? '';
            $this->adresse_retour = $data['adresse_retour'] ?? '';
            $this->code_postal_retour = $data['code_postal_retour'] ?? '';
            $this->ville_retour = $data['ville_retour'] ?? '';
        } else {
            $this->danger("Erreur lors du chargement du fournisseur.");
        }
    }
    ...
    public function save()
    {
        ...
        $response = Http::withToken($this->token)
            ->put(config('services.jwt.profile_endpoint') . '/fournisseur/fournisseur/'.$this->fournisseurId , $payload);
